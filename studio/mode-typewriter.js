...
const TypewriterMode = {
    ...
    render(ctx, canvas, img, params) {
        ...
        this.renderMockup(ctx, mockup, paddedWidth, targetWidth, targetHeight, params);
    },
     ...
     */
    renderMockup(ctx, mockup, mockupWidth, targetWidth, targetHeight, params) {
        ...
        for (let y = 0; y < targetHeight; y++) {
            for (let x = 0; x < targetWidth; x++) {
                const srcIdx = y * mockupWidth + x;
                const dstIdx = (y * targetWidth + x) * 4;

                // Mockup is 0-1 where 0=ink (black), 1=paper (white)
                const value = mockup[srcIdx];

                // Blend between ink and paper color
